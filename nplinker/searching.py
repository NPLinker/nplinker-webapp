...
class Searcher(object):
    ...
    def search(self, mode, text, use_re=True):
        ...
        if mode == SEARCH_OPT_BGC_STRAIN:
            results = [
                bgc for bgc in self.npl.bgcs
                if self.textmatch(needle, bgc.strain.id, use_re)
            ]
        elif mode == SEARCH_OPT_BGC_NAME:
            results = [
                bgc for bgc in self.npl.bgcs
                if self.textmatch(needle, bgc.name, use_re)
            ]
        elif mode == SEARCH_OPT_BGC_BIGSCAPE_CLASS:
            results = [
                bgc for bgc in self.npl.bgcs
                if self.textmatch(needle, bgc.bigscape_class, use_re)
            ]
        elif mode == SEARCH_OPT_BGC_PRODUCT_PREDICTION:
            results = [
                bgc for bgc in self.npl.bgcs if self.textmatch(
                    needle, ' '.join(bgc.product_prediction), use_re)
            ]
        elif mode == SEARCH_OPT_BGC_DESCRIPTION:
            results = [
                bgc for bgc in self.npl.bgcs
                if self.textmatch(needle, bgc.description, use_re)
            ]
        elif mode == SEARCH_OPT_GCF_STRAINS:
            results = [
                gcf for gcf in self.npl.gcfs if self.textmatch_list(
                    needle, [bgc.strain.id for bgc in gcf.bgcs], use_re)
            ]
        elif mode == SEARCH_OPT_GCF_ID:
            results = [
                gcf for gcf in self.npl.gcfs
                if self.textmatch(needle, str(gcf.gcf_id), use_re)
            ]
        elif mode == SEARCH_OPT_SPEC_STRAINS:
            results = [
                spec for spec in self.npl.spectra
                if self.textmatch_list(needle, [s.id
                                                for s in spec.strains], use_re)
            ]
        elif mode == SEARCH_OPT_SPEC_FAMILY:
            # if not in regex mode here, probably want to use exact matching
            results = [
                spec for spec in self.npl.spectra
                if self.textmatch(needle, spec.family, use_re, not use_re)
            ]
        elif mode == SEARCH_OPT_SPEC_ID:
            # if not in regex mode here, probably want to use exact matching
            results = [
                spec for spec in self.npl.spectra if self.textmatch(
                    needle, str(spec.spectrum_id), use_re, not use_re)
            ]
        elif mode == SEARCH_OPT_MOLFAM_FAMILY:
            results = [
                fam for fam in self.npl.molfams if self.textmatch(
                    needle, str(fam.family_id), use_re, not use_re)
            ]
