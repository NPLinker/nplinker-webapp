...
def on_session_created(session_context):
    ...
    if 'cutoff' in args:
        ...
        if val != nh.nplinker.bigscape_cutoff:
            print('*** Updating cutoff value to {} and reloading data'.format(
                val))
            nh.nplinker.load_data(new_bigscape_cutoff=val)
            nh.nplinker.process_dataset()
            nh.load_genomics()
    setattr(session_context._document, 'nh', nh)
    setattr(session_context._document, 'table_session_data',
            TableSessionData(nh.table_data))
