    ...
    return list(
        map(
            lambda t: {
                ...
                'constraintKeyName': t['options']['pk']
            }, filter(isTableVisible, tablesInfo)))
...
class Linker:
    ...
    def __init__(self, tablesInfo, dataset_id, path, do_init=False):
        ...
        self.tableColumns = {
            t['tableName']: [x for x in t['tableData'][0].keys()]
            for t in tablesInfo
        }
        self.columnIndices = {
            t['tableName']:
            {x: i
             for i, x in enumerate(t['tableData'][0].keys())}
            for t in tablesInfo
        }
    ...
    def getDefaultConstraints(self):
        ...
        constraints = {}
        for ct in ctables:
            constraints[ct['tableName']] = self.getKeys(
                self.tablesInfo, ct['tableName'], ct['constraintKeyName'])
        ...
        return list(
            map(
                lambda ti: {
                    ...
                    'fieldNames': list(ti['tableData'][0].keys())
                }, filter(isTableVisible, self.tablesInfo)))

    def getTableIdToIdColumnMap(self):
        return {
            t['tableName']: t['constraintKeyName']
            for t in getConstraintTablesConstraintKeyName(self.tablesInfo)
        }
    ...
    def query(self):
        ...
        if self.getTotalSelected() == 0:
            ...
            for tableName in tableNames:
                tableData[tableName] = self.sqlManager.simpleQuery(
                    tableName, self.tableColumns[tableName])
        ...
        print('createTempTable (totSel={}) took {:.2f}s'.format(
            self.getTotalSelected(),
            time.time() - t))
        ...
        for tableName in tableNames:
            tableData[tableName] = self.sqlManager.prefixQuery(
                tableName, self.tableColumns[tableName])
...
class SqlManager:
    ...
    def __init__(self, tablesInfo, path='linker.sqlite3', do_init=False):
        ...
        print('*** DB user_version is currently {}, expecting {}'.format(
            user_version, SqlManager.NPLINKER_USER_VERSION))
        if SqlManager.NPLINKER_USER_VERSION != user_version:
            print(
                '*** DB user_version mismatch ({} vs {}), regenerating content'
                .format(user_version, SqlManager.NPLINKER_USER_VERSION))
            do_init = True
        ...
        self.constraintTableConstraintKeyNames = getConstraintTablesConstraintKeyName(
            tablesInfo)
    ...
    def initialiseTables(self, tablesInfo):
        for t in tablesInfo:
            ...
            self.db.execute(sql)
            if 'pk' in t['options']:
                sql = 'CREATE UNIQUE INDEX IF NOT EXISTS {}_index ON {} ({})'.format(
                    t['tableName'], t['tableName'], t['options']['pk'])
                self.db.execute(sql)
            else:
                colnames = list(t['tableData'][0].keys())
                sql = 'CREATE UNIQUE INDEX IF NOT EXISTS {}_index on {} ({}, {})'.format(
                    t['tableName'], t['tableName'], colnames[0], colnames[1])
                self.db.execute(sql)

        # ensure we set the user_version value here
        self.db.execute('PRAGMA user_version = {}'.format(
            SqlManager.NPLINKER_USER_VERSION))

        # only add data if needed
        cur = self.db.execute('SELECT COUNT({}) FROM {}'.format(
            tablesInfo[0]['options']['pk'], tablesInfo[0]['tableName']))
        ...
        for t in tablesInfo:
            ...
            with self.db:
                ...
                stmt = 'INSERT INTO {} VALUES({})'.format(
                    t['tableName'], ','.join('?' for c in cols))
    ...
    def getFieldNames(self, tablesInfo):
        ...
        firstdatarows = map(
            lambda t: {
                'tableName': t['tableName'],
                'firstDataRow': t['tableData'][0]
            }, visible_tables)
        tabledata = map(
            lambda x: (x['tableName'], list(x['firstDataRow'].keys())),
            firstdatarows)
        all_cols = [
            '{}.{} AS {}_{}'.format(table, col, table, col)
            for table, cols in tabledata for col in cols
        ]
    ...
    def assembleInnerJoinStatementFromRelationship(self, relationship,
        ...
        def parseRelationship(r):
            if 'with' in r:
                if gen_to_met:
                    return 'INNER JOIN {} ON {}.{} = {}.{} '.format(
                        r['tableName'], r['with'], r['using'], r['tableName'],
                        r['using'])
                return 'INNER JOIN {} ON {}.{} = {}.{} '.format(
                    r['with'], r['tableName'], r['using'], r['with'],
                    r['using'])
            else:
                return ''
    ...
    def makeSelectClause(self, tablesInfo, gen_to_met):
        ...
        if gen_to_met:
            return 'SELECT {} FROM {}'.format(fieldNames,
                                              tablesInfo[-1]['tableName'])
    ...
    def makeInnerJoinClause(self, gen_to_met):
        if gen_to_met:
            return ' '.join(
                map(
                    lambda x: self.assembleInnerJoinStatementFromRelationship(
                        x, gen_to_met), reversed(self.tableRelationships)))
        return ' '.join(
            map(
                lambda x: self.assembleInnerJoinStatementFromRelationship(
                    x, gen_to_met), self.tableRelationships))
    ...
    def makeWhereClause(self, tablesInfo, skipConstraints):
        ...
        for i, value in enumerate(whereSubClauses):
            if not skipConstraints[i]:
                selectedWhereSubClauses.append('{} IN @(?)'.format(
                    whereSubClauses[i]))
    ...
    def makeWhereSubClauses(self):
        return list(
            map(
                lambda t: '{}.{}'.format(t['tableName'], t['constraintKeyName']
                                         ),
                self.constraintTableConstraintKeyNames))

    def simpleQuery(self, tableName, tableFieldNames):
        sqlQuery = 'SELECT ' + ', '.join(tableFieldNames) + ' FROM {}'.format(
            tableName)
    ...
    def prefixQuery(self, tableName, tableFieldNames):
        ...
        sqlQuery = 'SELECT DISTINCT ' + ', '.join(
            tableFieldNames) + ' FROM tempTable'
        ...
        for row in resultset:
            data.append(
                {f.replace(prefix, ''): row[f]
                 for f in tableFieldNames})

        return data

    def createTempTable(self, tablesInfo, constraints):
        constraintTableNames = list(
            map(lambda t: t['tableName'],
                self.constraintTableConstraintKeyNames))
        unpackedConstraints = list(
            map(lambda n: constraints[n], constraintTableNames))
        skipConstraints = []
        selectedConstraints = []
        for i, uc in enumerate(unpackedConstraints):
            tableName = constraintTableNames[i]
            myTable = list(
                filter(lambda t: t['tableName'] == tableName, tablesInfo))[0]
        ...
        while True:
            ...
            sqlQuery = sqlQuery.replace(
                '@(?)', '({})'.format(','.join(
                    ['?' for i in range(len(selectedConstraints[c]))])), 1)
            c += 1
        ...
        # is happening
        selectedConstraints = list(
            map(int, itertools.chain.from_iterable(selectedConstraints)))
...
if __name__ == "__main__":
    ...
    link_indices = [(1, 1), (1, 2), (1, 3), (2, 4), (2, 5), (3, 6), (4, 7),
                    (5, 8), (5, 9), (5, 10)]
    mf_spectra = create_links(mf_df, spectra_df, 'mf_pk', 'spectra_pk',
                              link_indices)

    link_indices = [(1, 1), (2, 2), (1, 3), (3, 4), (3, 5), (5, 7)]
    spectra_bgc = create_links(spectra_df, bgc_df, 'spectra_pk', 'bgc_pk',
                               link_indices)

    link_indices = [(1, 1), (2, 1), (3, 1), (4, 1), (5, 2), (6, 3), (7, 4),
                    (8, 5), (9, 5), (10, 5)]
    ...
    table_info = get_table_info(mf_df, mf_spectra, spectra_df, spectra_bgc,
                                bgc_df, bgc_gcf, gcf_df)
    data_sources = {
        'mf_table': mf_ds,
