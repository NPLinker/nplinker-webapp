...
SCO_MODE_NAMES = [
    'BGCs to Spectra', 'BGCs to MolFams', 'GCFs to Spectra', 'GCFs to MolFams',
...
TMPL = open(
    os.path.join(os.path.dirname(__file__),
                 'templates/tmpl.basic.py.html')).read()

# same as above but with an extra pair of parameters for the "onclick" handler and button id
TMPL_ON_CLICK = open(
    os.path.join(os.path.dirname(__file__),
                 'templates/tmpl.onclick.py.html')).read()

TMPL_SEARCH = open(
    os.path.join(os.path.dirname(__file__),
                 'templates/tmpl.basic.search.py.html')).read()

TMPL_SEARCH_ON_CLICK = open(
    os.path.join(os.path.dirname(__file__),
                 'templates/tmpl.onclick.search.py.html')).read()
...
class ScoringHelper(object):
    ...
    GENOMICS_LOOKUP = {
        GENOMICS_MODE_BGC: [SCO_MODE_BGC_SPEC, SCO_MODE_BGC_MOLFAM],
    ...
    def _update(self):
        if self.from_genomics:
            self.mode = ScoringHelper.GENOMICS_LOOKUP[self.genomics_mode][
                self.metabolomics_mode]
        else:
            self.mode = ScoringHelper.METABOLOMICS_LOOKUP[
                self.metabolomics_mode]
    ...
    def generate_scoring_objects(self, bgcs, spectra, tsne_id):
        ...
        if self.mode == SCO_MODE_BGC_SPEC or self.mode == SCO_MODE_BGC_MOLFAM:
            ...
            scoring_objs = set(
                [parent for bgc in bgcs for parent in bgc.parents])
            ...
            for bgc in bgcs:
                ...
                gcfs_for_bgc = [
                    gcf for gcf in gcfs_for_bgc if gcf in available_gcfs
                ]
            ...
            print('Selection of {} BGCs => {} GCFs'.format(
                len(bgcs), len(scoring_objs)))
            self.gcfs = list(scoring_objs)
...
class NPLinkerBokeh(object):
    ...
    def generate_gcf_spec_result(self, pgindex, gcf, link_data):
        ...
        if self.scoring_objects[
                'metcalf'] in self.score_helper.scoring_results.methods:
            sorted_links = self.score_helper.scoring_results.get_sorted_links(
                self.scoring_objects['metcalf'], gcf)
        else:
        ...
        for link in sorted_links:
            ...
            score = '[' + ', '.join(
                '{}={}'.format(m.name, m.format_data(link[m]))
                for m in link.methods) + ']'
            ...
            for spec in specs:
                spec_title = 'Spectrum(parent_mass={:.4f}, id={}), scores <strong>{}</strong>, shared strains=<strong>{}</strong>'.format(
                    spec.parent_mz, spec.spectrum_id, score,
                    len(link.shared_strains))
                if spec.has_annotations():
                    spec_title += ', # annotations={}'.format(
                ...
                spec_body += '<center><canvas id="{}"></canvas></center>'.format(
                    spec_plot_id)
                span_badges = '&nbsp|&nbsp;'.join([
                    '<span class="badge badge-info">{}</span>'.format(m.name)
                    for m in link.methods
                ])

                # note annoying escaping required here, TODO better way of doing this?
                spec_onclick = 'setupPlot(\'{}\', \'{}\', \'{}\');'.format(
                    spec_btn_id, spec_plot_id, spec.to_jcamp_str())

                hdr_color = 'ffe0b5'
                if len(spec.annotations) > 0:
                    hdr_color = 'ffb5e0'
                body += TMPL_ON_CLICK.format(
                    hdr_id=spec_hdr_id,
                    hdr_color=hdr_color,
                    btn_target=spec_body_id,
                    btn_onclick=spec_onclick,
                    btn_id=spec_btn_id,
                    btn_text=spec_title,
                    span_badges=span_badges,
                    body_id=spec_body_id,
                    body_parent='accordion_gcf_{}'.format(pgindex),
                    body_body=spec_body)
                j += 1
    ...
    def generate_spec_gcf_result(self, pgindex, spec, link_data):
        ...
        if self.scoring_objects[
                'metcalf'] in self.score_helper.scoring_results.methods:
            sorted_links = self.score_helper.scoring_results.get_sorted_links(
                self.scoring_objects['metcalf'], spec)
        else:
            sorted_links = link_data.values()
        ...
        for link in sorted_links:
            ...
            gcf_title = 'GCF(id={}), scores <strong>{}</strong>, shared strains=<strong>{}</strong>'.format(
                gcf.gcf_id, score, len(link.shared_strains))
            ...
            body += TMPL.format(
                hdr_id=gcf_hdr_id,
                hdr_color='ffe0b5',
                btn_target=gcf_body_id,
                btn_text=gcf_title,
                span_badges=span_badges,
                body_id=gcf_body_id,
                body_parent='accordion_spec_{}'.format(pgindex),
                body_body=gcf_body)
    ...
    def update_results_gcf_spec(self):
        ...
        for gcf, link_data in self.score_helper.scoring_results.links.items():
            ...
            title = '{} spectra linked to GCF(id={})'.format(
                link_count, gcf.gcf_id)
            ...
            # by number of links
            unsorted.append((link_count,
                             TMPL.format(hdr_id=hdr_id,
                                         hdr_color='adeaad',
                                         btn_target=body_id,
                                         btn_text=title,
                                         span_badges='',
                                         body_id=body_id,
                                         body_parent='accordionResults',
                                         body_body=body)))
            pgindex += 1

        content = '<h4>GCFs={}, total # linked objects={}</h4>'.format(
            len(self.score_helper.scoring_results),
            sum(x[0] for x in unsorted))
    ...
    def update_results_spec_gcf(self):
        ...
        for spec, link_data in self.score_helper.scoring_results.links.items():
            ...
            body += '<center><canvas id="{}"></canvas></center>'.format(
                plot_id)
            # note annoying escaping required here, TODO better way of doing this?
            onclick = 'setupPlot(\'{}\', \'{}\', \'{}\');'.format(
                btn_id, plot_id, spec.to_jcamp_str())
            ...
            unsorted.append(
                (len(link_data),
                 TMPL_ON_CLICK.format(hdr_id=hdr_id,
                                      hdr_color='b4c4e8',
                                      btn_target=body_id,
                                      btn_onclick=onclick,
                                      btn_id=btn_id,
        ...
        content = '<h4>Spectra={}, total # linked GCFs={}</h4>'.format(
            len(self.score_helper.scoring_results), len(all_gcfs))
    ...
    def update_results(self, div):
        ...
        if self.score_helper.mode == SCO_MODE_BGC_SPEC or self.score_helper.mode == SCO_MODE_GCF_SPEC:
            ...
            self.debug_log('Unknown scoring mode selected! {}'.format(
                self.score_helper.mode))
    ...
    def display_links(self, scoring_results, mode, div, include_only=None):
        ...
        if len(scoring_results) > 0:
            ...
            self.update_alert(
                '{} objects with links found'.format(
                    len(self.score_helper.scoring_results)), 'primary')
        else:
    ...
    def scoring_method_callback(self, attr, old, new):
        ...
        for i in range(len(self.scoring_methods)):
            self.scoring_method_buttons[
                self.scoring_methods[i]].visible = i in new

    def get_scoring_mode_text(self):
        return SCO_MODE_NAMES[self.score_helper.mode]

    def sco_mode_changed(self):
        self.update_alert('Scoring mode is now <strong>{}</strong>'.format(
            self.get_scoring_mode_text()))

    def update_alert(self, msg, alert_class='primary'):
        self.alert_div.text = '<div class="alert alert-{}" role="alert">{}</div>'.format(
            alert_class, msg)
    ...
    def search_type_callback(self, attr, old, new):
        print('Search type is now: {}={}'.format(new,
                                                 SEARCH_OPTIONS.index(new)))

    def generate_bgc_info(self, bgc):
        bgc_body = '<ul>'
        for attr in [
                'strain', 'name', 'bigscape_class', 'product_prediction',
                'description'
        ]:
            bgc_body += '<li><strong>{}</strong>: {}</li>'.format(
                attr, getattr(bgc, attr))
        bgc_body += '</ul>'
        return bgc_body
    ...
    def generate_gcf_info(self, gcf, link=None):
        ...
        gcf_body = '<ul class="nav nav-tabs" id="gcf_{}_tabs" role="tablist">'.format(
            gcf.id)
        gcf_body += '<li class="nav-item"><a class="nav-link active" id="gcf_{}_main_tab" data-toggle="tab" href="#gcf_{}_main" role="tab">Main</a></li>'.format(
            gcf.id, gcf.id)
        gcf_body += '<li class="nav-item"><a class="nav-link" id="gcf_{}_bgcs_tab" data-toggle="tab" href="#gcf_{}_bgcs" role="tab">BGCs</a></li>'.format(
            gcf.id, gcf.id)
        if rosetta_obj is not None:
            gcf_body += '<li class="nav-item"><a class="nav-link" id="gcf_{}_rosetta_tab" data-toggle="tab" href="#gcf_{}_rosetta" role="tab">Rosetta hits</a></li>'.format(
                gcf.id, gcf.id)
        gcf_body += '</ul><div class="tab-content" id="gcf_{}_tab_content">'.format(
            gcf.id)

        # start of main tab content
        gcf_body += '<div class="tab-pane show active" id="gcf_{}_main" role="tabpanel">'.format(
            gcf.id)
        gcf_body += '<ul>'
        for attr in ['id', 'gcf_id', 'bigscape_class']:
            gcf_body += '<li><strong>{}</strong>: {}</li>'.format(
                attr, getattr(gcf, attr))

        # add strain information
        if shared_strains is not None and len(shared_strains) > 0:
            gcf_body += '<li><strong>strains (total={}, shared={})</strong>: '.format(
                len(gcf.bgcs), len(shared_strains))

            non_shared = [
                s.strain for s in gcf.bgcs if s.strain not in shared_strains
            ]

            for s in shared_strains:
                gcf_body += '<span style="background-color: #AAFFAA">{}</span>, '.format(
                    s.id)
            for s in non_shared:
                gcf_body += '<span style="background-color: #DDDDDD">{}</span>, '.format(
                    s.id)

            gcf_body += '</li>'
        else:
            gcf_body += '<li><strong>strains (total={}, shared=0)</strong>: '.format(
                len(gcf.bgcs))
        ...
        # BGC tab content
        fields = [
            'name', 'strain', 'description', 'bigscape_classes',
            'product_prediction'
        ]
        gcf_body += '<div class="tab-pane" id="gcf_{}_bgcs" role="tabpanel">'.format(
            gcf.id)
        gcf_body += 'Type to filter: <input type="text" onkeyup="onChangeBGCTable(\'#gcf_{}_bgc_table\', \'#gcf_{}_bgc_search\')" id="gcf_{}_bgc_search">'.format(
            gcf.id, gcf.id, gcf.id)
        gcf_body += '<table class="table table-responsive table-striped" id="gcf_{}_bgc_table"><thead><tr>'.format(
            gcf.id)
        gcf_body += ''.join('<th scope="col">{}</th>'.format(x)
                            for x in fields)
        gcf_body += '</thead><tbody>'
        for bgc in gcf.bgcs:
            gcf_body += '<tr><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>'.format(
                *list(getattr(bgc, x) for x in fields))
        ...
        if rosetta_obj is not None:
            rosetta_fields = [
                'BGC name', 'BGC strain', 'MiBIG ID', 'BGC score',
                'Spectrum ID', 'GNPS ID', 'Spectrum score'
            ]
            gcf_body += '<div class="tab-pane" id="gcf_{}_rosetta" role="tabpanel">'.format(
                gcf.id)
            gcf_body += 'Type to filter: <input type="text" onkeyup="onChangeRosettaTable(\'#gcf_{}_rosetta_table\', \'#gcf_{}_rosetta_search\')" id="gcf_{}_rosetta_search">'.format(
                gcf.id, gcf.id, gcf.id)
            gcf_body += '<table class="table table-responsive table-striped" id="gcf_{}_rosetta_table"><thead><tr>'.format(
                gcf.id)
            gcf_body += ''.join('<th scope="col">{}</th>'.format(x)
                                for x in rosetta_fields)
            gcf_body += '</thead><tbody>'
            for hit in link.data(rosetta_obj):
                bgc_url = '<a href="https://mibig.secondarymetabolites.org/repository/{}" target="_blank">{}</a>'.format(
                    hit.mibig_id, hit.mibig_id)
                gcf_body += '<tr><td>{}</td><td>{}</td><td>{}</td><td>{:.3f}</td>'.format(
                    hit.bgc.name, hit.bgc.strain, bgc_url, hit.bgc_match_score)
                spec_url = '<a href="{}" target="_blank">{}</a>'.format(
                    gnps_url(hit.gnps_id, 'spectrum'), hit.gnps_id)
                gcf_body += '<td>{}</td><td>{}</td><td>{:.3f}</td></tr>'.format(
                    hit.spec.spectrum_id, spec_url, hit.spec_match_score)
            gcf_body += '</tbody></table>'
            gcf_body += '</div>'
    ...
    def generate_spec_info(self, spec, link=None):
        ...
        spec_body = '<ul class="nav nav-tabs" id="spec_{}_tabs" role="tablist">'.format(
            spec.id)
        spec_body += '<li class="nav-item"><a class="nav-link active" id="spec_{}_main_tab" data-toggle="tab" href="#spec_{}_main" role="tab">Main</a></li>'.format(
            spec.id, spec.id)
        if len(spec.annotations) > 0:
            spec_body += '<li class="nav-item"><a class="nav-link" id="spec_{}_anno_tab" data-toggle="tab" href="#spec_{}_anno" role="tab">Annotations</a></li>'.format(
                spec.id, spec.id)
        if rosetta_obj is not None:
            spec_body += '<li class="nav-item"><a class="nav-link" id="spec_{}_rosetta_tab" data-toggle="tab" href="#spec_{}_rosetta" role="tab">Rosetta hits</a></li>'.format(
                spec.id, spec.id)
        spec_body += '</ul><div class="tab-content" id="spec_{}_tab_content">'.format(
            spec.id)

        # main tab content
        spec_body += '<div class="tab-pane show active" id="spec_{}_main" role="tabpanel">'.format(
            spec.id)
        spec_body += '<ul>'
        for attr in [
                'id', 'spectrum_id', 'family', 'rt', 'total_ms2_intensity',
                'max_ms2_intensity', 'n_peaks', 'precursor_mz', 'parent_mz'
        ]:
            spec_body += '<li><strong>{}</strong>: {}</li>'.format(
                attr, getattr(spec, attr))

        # TODO possibly going to need updated later
        if 'ATTRIBUTE_SampleType' in spec.metadata:
            spec_body += '<li><strong>SampleType</strong>: {}</li>'.format(
                ', '.join(spec.metadata['ATTRIBUTE_SampleType']))

        # add strain information (possibly empty)
        if link is not None and len(link.shared_strains) > 0:
            spec_body += '<li><strong>strains (total={}, shared={})</strong>: '.format(
                len(spec.strains), len(link.shared_strains))

            non_shared = [
                s for s in spec.strains if s not in link.shared_strains
            ]

            for s in link.shared_strains:
                spec_body += '<span style="background-color: #AAFFAA">{} ({})</span>, '.format(
                    s.id, spec.get_growth_medium(s))
            for s in non_shared:
                spec_body += '<span style="background-color: #DDDDDD">{} ({})</span>, '.format(
                    s.id, spec.get_growth_medium(s))

            spec_body += '</li>'
        else:
            spec_body += '<li><strong>strains (total={}, shared=0)</strong>: '.format(
                len(spec.strains))

            for s in spec.strains:
                spec_body += '<span>{} ({})</span>, '.format(
                    s.id, spec.get_growth_medium(s))
        ...
        # annotations tab
        if len(spec.annotations) > 0:
            spec_body += '<div class="tab-pane" id="spec_{}_anno" role="tabpanel">'.format(
                spec.id)
            ...
            for anno_src, anno_list in spec.annotations.items():
                spec_body += '<strong>{} annotations:</strong><ul>'.format(
                    anno_src)
                for item in anno_list:
                    ...
                    for k, v in item.items():
                        if k.endswith('_url'):
                            v = '<a href="{}" target="_blank">{}</a>'.format(
                                v, k)
                        elif v.startswith('CCMSLIB'):
                            v = '<a href="{}" target="_blank">{}</a>'.format(
                                gnps_url(v), v)
                        content.append(
                            '<strong><span class="annotation">{}</span></strong> = {}'
                            .format(k, v))
        ...
        if rosetta_obj is not None:
            ...
            spec_body += '<div class="tab-pane" id="spec_{}_rosetta" role="tabpanel">'.format(
                spec.id)
            spec_body += 'Type to filter: <input type="text" onkeyup="onChangeRosettaTable(\'#spec_{}_rosetta_table\', \'#spec_{}_rosetta_search\')" id="spec_{}_rosetta_search">'.format(
                spec.id, spec.id, spec.id)
            spec_body += '<table class="table table-responsive table-striped" id="spec_{}_rosetta_table"><thead><tr>'.format(
                spec.id)
            spec_body += ''.join('<th scope="col">{}</th>'.format(x)
                                 for x in rosetta_fields)
            spec_body += '</thead><tbody>'
            for hit in link.data(rosetta_obj):
                bgc_url = '<a href="https://mibig.secondarymetabolites.org/repository/{}" target="_blank">{}</a>'.format(
                    hit.mibig_id, hit.mibig_id)
                spec_body += '<tr><td>{}</td><td>{}</td><td>{}</td><td>{:.3f}</td>'.format(
                    hit.bgc.name, hit.bgc.strain, bgc_url, hit.bgc_match_score)
                spec_url = '<a href="{}" target="_blank">{}</a>'.format(
                    gnps_url(hit.gnps_id, 'spectrum'), hit.gnps_id)
                spec_body += '<td>{}</td><td>{}</td><td>{:.3f}</td></tr>'.format(
                    hit.spec.spectrum_id, spec_url, hit.spec_match_score)
    ...
    def generate_search_output_bgc(self, bgcs):
        ...
        for i, bgc in enumerate(bgcs):
            ...
            body += TMPL_SEARCH.format(hdr_id=bgc_hdr_id,
                                       hdr_color='dddddd',
                                       btn_target=bgc_body_id,
                                       btn_text=bgc_title,
                                       result_index=str(i),
                                       body_id=bgc_body_id,
                                       body_parent='accordionSearch',
                                       body_body=bgc_body)
    ...
    def generate_search_output_gcf(self, gcfs):
        ...
        for i, gcf in enumerate(gcfs):
            ...
            gcf_title = 'GCF(gcf_id={}, strains={})'.format(
                gcf.gcf_id, len(gcf.bgcs))
            gcf_body = self.generate_gcf_info(gcf)
            body += TMPL_SEARCH.format(hdr_id=gcf_hdr_id,
                                       hdr_color='dddddd',
                                       btn_target=gcf_body_id,
                                       btn_text=gcf_title,
                                       result_index=str(i),
    ...
    def generate_search_output_spec(self, spectra):
        ...
        for i, spec in enumerate(spectra):
            ...
            spec_title = 'Spectrum(parent_mass={:.4f}, id={}, strains={})'.format(
                spec.parent_mz, spec.spectrum_id, len(spec.strains))
            ...
            body += TMPL_SEARCH_ON_CLICK.format(hdr_id=spec_hdr_id,
                                                hdr_color='dddddd',
                                                btn_target=spec_body_id,
                                                btn_onclick=spec_onclick,
                                                btn_id=spec_btn_id,
    ...
    def generate_search_output(self, results):
        ...
        hdr = '<h4><button type="button" class="btn btn-info" onclick="toggleSearchSelect()">{} results found, click to select/deselect all</button></h4>'.format(
            len(results))
        self.search_div_header.text = hdr
    ...
    def search_button_callback(self):
        self.generate_search_output(
            self.searcher.search(self.search_type.value,
                                 self.search_input.value,
                                 len(self.search_regex.active) == 1))
    ...
    def search_score_button_callback(self):
        ...
        selected_indices = list(
            map(int, self.confirm_response.data['selected'][0]))
        ...
        if isinstance(results[0], BGC):
            ...
            print('current scoring mode gen? {}'.format(
                self.score_helper.from_genomics))
            # now select bgc mode
            ...
            for bgc in results:
                for i in range(
                        len(self.table_session_data.bgc_ds.data['bgc_pk'])):
                    if bgc.id == self.table_session_data.bgc_ds.data['bgc_pk'][
                            i]:
                        indices.append(i)
                        break
        ...
        # if results are GCFs, switch to genomics mode and select the GCF mode
            ...
            print('current scoring mode met? {}'.format(
                not self.score_helper.from_genomics))
            ...
            for spec in results:
                for i in range(
                        len(self.table_session_data.spec_ds.data['spectrum_id']
                            )):
                    if spec.spectrum_id == self.table_session_data.spec_ds.data[
                            'spectrum_id'][i]:
                        indices.append(i)
                        break
    ...
    def gnps_params_select_callback(self, attr, old, new):
        self.gnps_params_value.text = '<strong>{}</strong> = {}'.format(
            new, self.nh.nplinker.gnps_params[new])

    def current_scoring_method_names(self):
        print('NAMES', [
            self.scoring_methods[x]
            for x in self.scoring_method_checkboxes.active
        ])
        return [
            self.scoring_methods[x]
            for x in self.scoring_method_checkboxes.active
    ...
    def current_scoring_methods(self):
        print('METHODS', [
            self.scoring_objects[x]
            for x in self.current_scoring_method_names()
        ])
        return [
            self.scoring_objects[x]
            for x in self.current_scoring_method_names()
    ...
    def tables_score_met_callback(self):
        ...
        """
        print('Currently selected spectra: {}'.format(
            len(self.table_session_data.spec_ds.data['spectrum_id'])))
        print('Actual selections: {}'.format(
            len(self.table_session_data.spec_ds.selected.indices)))

        if len(self.table_session_data.spec_ds.selected.indices) == 0 and len(
                self.table_session_data.molfam_ds.selected.indices) == 0:
        ...
        if len(self.table_session_data.spec_ds.selected.indices) > 0:
            ...
            for index in self.table_session_data.spec_ds.selected.indices:
                sid = self.table_session_data.spec_ds.data['spec_pk'][
                    index]  # the internal nplinker ID
                if sid == NA_ID:
                    continue
                selected_spectra.append(self.nh.nplinker.spectra[int(sid)])
        else:
            for sid in self.table_session_data.spec_ds.data[
                    'spec_pk']:  # internal nplinker IDs
                if sid == NA_ID:
                    continue
        ...
        results = self.nh.nplinker.get_links(
            selected_spectra,
            current_methods,
            and_mode=self.score_helper.method_and_mode)
        ...
        table_metcalf_scores = [
            '-' for gcf_id in self.table_session_data.gcf_ds.data['gcf_id']
        ]
        ...
        if metcalf_obj in current_methods:
            ...
            visible_gcfs = list(
                map(int, self.table_session_data.gcf_ds.data['gcf_id']))
        ...
        self.table_session_data.gcf_ds.data[
            'metcalf_score'] = table_metcalf_scores
        ...
        include_only = set([
            self.nh.nplinker.gcfs[int(gcf_id)]
            for gcf_id in self.table_session_data.gcf_ds.data['gcf_pk']
            if gcf_id != '-'
        ])

        # FINALLY, display the link information
        self.display_links(results,
                           mode=SCO_MODE_SPEC_GCF,
                           div=self.results_div,
    ...
    def tables_score_gen_callback(self):
        ...
        print('Currently selected bgcs: {}'.format(
            len(self.table_session_data.bgc_ds.data['bgc_pk'])))
        print('Actual selections: {}'.format(
            len(self.table_session_data.bgc_ds.selected.indices)))
        ignore_hybrid_bgcs = len(
            self.hybrid_scoring_control_checkbox.active) == 1

        if len(self.table_session_data.bgc_ds.selected.indices) == 0 and len(
                self.table_session_data.gcf_ds.selected.indices) == 0:
        ...
        if len(self.table_session_data.bgc_ds.selected.indices) > 0:
            ...
            for index in self.table_session_data.bgc_ds.selected.indices:
                bgcid = self.table_session_data.bgc_ds.data['bgc_pk'][
                    index]  # internal nplinker ID
                if bgcid == NA_ID:
                    continue
            ...
            for index in self.table_session_data.gcf_ds.selected.indices:
                gcfid = self.table_session_data.gcf_ds.data['gcf_pk'][
                    index]  # internal nplinker ID
                tmp_gcfs.append(self.nh.nplinker.gcfs[gcfid])

            for bgcid in self.table_session_data.bgc_ds.data[
                    'bgc_pk']:  # internal nplinker IDs
                if bgcid == NA_ID:
                    continue
                bgc = self.nh.nplinker.bgcs[int(bgcid)]
                selected_bgcs.append(bgc)
        ...
        table_metcalf_scores = [
            '-' for spec_id in
            self.table_session_data.spec_ds.data['metcalf_score']
        ]
        # check if metcalf scoring is enabled at the moment
        if metcalf_obj in current_methods:
            ...
            visible_spectra = list(
                map(int, self.table_session_data.spec_ds.data['spectrum_id']))
        ...
        self.table_session_data.spec_ds.data[
            'metcalf_score'] = table_metcalf_scores
        ...
        # TODO MolFam mode...
        include_only = set([
            self.nh.nplinker.spectra[int(spec_id)]
            for spec_id in self.table_session_data.spec_ds.data['spec_pk']
            if spec_id != '-'
        ])

        # FINALLY, display the link information
        self.display_links(results,
                           mode=SCO_MODE_BGC_SPEC,
                           div=self.results_div,
                           include_only=include_only)
    ...
    def bokeh_layout(self):
        widgets = []
        self.results_div = Div(text=RESULTS_PLACEHOLDER,
                               name='results_div',
                               width_policy='max',
                               height_policy='fit')
        ...
        self.scoring_objects = {
            m: self.nh.nplinker.scoring_method(m)
            for m in self.scoring_methods
        }

        # checkbox list of scoring methods to enable/disable them
        self.scoring_method_checkboxes = CheckboxGroup(
            labels=[m for m in self.scoring_methods],
            active=[0],
            name='scoring_method_checkboxes')
        self.scoring_method_checkboxes.on_change('active',
                                                 self.scoring_method_callback)
        widgets.append(self.scoring_method_checkboxes)

        # buttons to show the modal dialogs for configuring each scoring method
        self.scoring_method_buttons = {}
        self.scoring_method_buttons['metcalf'] = Button(
            label='Configure Metcalf scoring',
            name='metcalf_scoring_button',
            button_type='primary')
        self.scoring_method_buttons['rosetta'] = Button(
            label='Configure rosetta scoring',
            name='rosetta_scoring_button',
            button_type='primary',
            visible=False)
        ...
        self.scoring_method_buttons['metcalf'].js_on_click(
            CustomJS(args=dict(modal_name='metcalfModal'),
                     code=show_modal_callback))
        self.scoring_method_buttons['rosetta'].js_on_click(
            CustomJS(args=dict(modal_name='rosettaModal'),
                     code=show_modal_callback))

        # metcalf config objects
        self.metcalf_standardised = RadioGroup(
            labels=['Basic Metcalf scoring', 'Standardised Metcalf scoring'],
            name='metcalf_standardised',
            active=1 if self.scoring_objects['metcalf'].standardised else 0,
            sizing_mode='scale_width')
        self.metcalf_standardised.on_change('active',
                                            self.metcalf_standardised_callback)
        widgets.append(self.metcalf_standardised)
        self.metcalf_cutoff = Slider(
            start=0,
            end=50,
            value=int(self.scoring_objects['metcalf'].cutoff),
            step=1,
            title='cutoff value',
            name='metcalf_cutoff')
        ...
        # same for rosetta
        self.rosetta_spec_cutoff = Slider(
            start=0,
            end=1,
            step=0.1,
            value=self.scoring_objects['rosetta'].bgc_score_cutoff,
            name='rosetta_spec_cutoff',
            title='spectral score cutoff value')
        self.rosetta_spec_cutoff.on_change('value',
                                           self.rosetta_spec_cutoff_callback)
        self.rosetta_bgc_cutoff = Slider(
            start=0,
            end=1,
            step=0.1,
            value=self.scoring_objects['rosetta'].bgc_score_cutoff,
            name='rosetta_bgc_cutoff',
            title='BGC score cutoff value')
        self.rosetta_bgc_cutoff.on_change('value',
                                          self.rosetta_bgc_cutoff_callback)
        widgets.append(self.rosetta_spec_cutoff)
        widgets.append(self.rosetta_bgc_cutoff)

        self.rosetta_dl_button = Button(
            label='Download all Rosetta hits for this dataset as a CSV file',
            name='rosetta_dl_button')
        ...
        rosetta_path = os.path.join(self.nh.nplinker.root_dir, 'rosetta',
                                    'rosetta_hits.csv')
        if os.path.exists(rosetta_path):
            with open(rosetta_path, 'r') as f:
                rosetta_csv_text = f.read()
        self.rosetta_dl_button.js_on_click(
            CustomJS(args=dict(filetext=rosetta_csv_text),
                     code=rosetta_download_code))
        widgets.append(self.rosetta_dl_button)

        # and/or mode selection
        self.scoring_mode_toggle = RadioButtonGroup(labels=[
            'Show results from all methods (AND mode)',
            'Show results from any method (OR mode)'
        ],
                                                    active=0,
                                                    name='scoring_mode_toggle',
                                                    sizing_mode='scale_width')
        self.scoring_mode_toggle.on_change('active',
                                           self.scoring_mode_toggle_callback)
        widgets.append(self.scoring_mode_toggle)

        metcalf_info_div = Div(
            name='metcalf_info',
            sizing_mode='scale_width',
            text='Tables contain objects with Metcalf score > {:.1f}'.format(
                self.nh.nplinker._loader.webapp_scoring_cutoff()))
        self.metcalf_info = wrap_popover(
            metcalf_info_div, create_popover(*TOOLTIP_TABLES_FILTERING),
            'metcalf_info')
        widgets.append(self.metcalf_info)

        # for debug output etc
        self.debug_div = Div(text="",
                             name='debug_div',
                             sizing_mode='scale_width')
        ...
        self.search_button = Button(name='search_button',
                                    label='Search',
                                    button_type='primary',
                                    sizing_mode='scale_width')
        ...
        self.search_score_button = Button(
            name='search_score_button',
            label='Run scoring on selected results',
            button_type='warning',
            sizing_mode='scale_width')
        self.search_score_button.on_click(self.search_score_button_callback)
        self.confirm_response = ColumnDataSource(data={
            'resp': [None],
            'selelected': [[]]
        })
        self.search_score_button.js_on_click(
            CustomJS(args=dict(resp=self.confirm_response),
                     code="""
        ...
        self.search_div_results = Div(text='',
                                      sizing_mode='scale_both',
                                      name='search_div_results')
        widgets.append(self.search_div_results)

        self.search_div_header = Div(text='',
                                     sizing_mode='scale_height',
                                     name='search_div_header')
        widgets.append(self.search_div_header)

        self.search_type = Select(options=SEARCH_OPTIONS,
        ...
        self.search_regex = CheckboxGroup(labels=['Use regexes?'],
                                          active=[0],
                                          name='search_regex',
                                          sizing_mode='scale_width')
        widgets.append(self.search_regex)

        self.dataset_description_pre = PreText(
            text=self.nh.nplinker.dataset_description,
            sizing_mode='scale_both',
            name='dataset_description_pre')
        ...
        self.gnps_params_select = Select(options=list(gnps_params.keys()),
                                         value=defval,
                                         name='gnps_params_select',
                                         css_classes=['nolabel'],
                                         sizing_mode='scale_height')
        self.gnps_params_select.on_change('value',
                                          self.gnps_params_select_callback)
        widgets.append(self.gnps_params_select)

        if defval is not None:
            deftext = '<strong>{}</strong> = {}'.format(
                defval, self.nh.nplinker.gnps_params[defval])
        else:
            deftext = ''
        self.gnps_params_value = Div(text=deftext,
                                     sizing_mode='scale_height',
                                     name='gnps_params_value')
        ...
        other_info += '<tr><td>{}</td><td>{}</td></tr>'.format(
            'BiGSCAPE clustering cutoff', self.nh.nplinker.bigscape_cutoff)
        other_info += '</tbody></table>'
        self.other_info_div = Div(text=other_info,
                                  sizing_mode='scale_height',
                                  name='other_info_div')
        ...
        self.table_session_data.tables_score_met.on_click(
            self.tables_score_met_callback)
        self.table_session_data.tables_score_gen.on_click(
            self.tables_score_gen_callback)

        # and reset button
        self.table_session_data.tables_reset_button.on_click(
            self.tables_reset_callback)

        self.hidden_alert_thing = PreText(text='',
                                          name='hidden_alert',
                                          visible=False)
        self.hidden_alert_thing.js_on_change(
            'text',
            CustomJS(code='if(cb_obj.text.length > 0) alert(cb_obj.text);',
                     args={}))
        widgets.append(self.hidden_alert_thing)

        # hybrid scoring control
        self.hybrid_scoring_control_checkbox = CheckboxGroup(
            sizing_mode='scale_width',
            labels=['Treat hybrid BGCs as non-hybrid during scoring'],
            active=[0],
            name='hybrid_scoring_control_checkbox')
        self.hybrid_scoring_control_checkbox_wrapper = wrap_popover(
            self.hybrid_scoring_control_checkbox,
            create_popover(*TOOLTIP_HYBRID_SCORING),
            'hybrid_scoring_control_checkbox',
            sizing_mode='scale_width')
